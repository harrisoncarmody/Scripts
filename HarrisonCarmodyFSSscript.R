...
dat <-read.csv("C:/Users/Harrison Carmody/Documents/Uni Documents/Masters/Project/Spanish Mackerel Depredation/Results/Raw/CombinedlDailyQueries by SessionID to 2017HC SST Edit.csv")%>%
  
  select(c(Decimal.median.time,Hours,Month,Year,Block,Zone,Skipper,sst,Fraction.moon.illuminated,Fishing.pressure.all.years,NumberCaught,Method,NumberMackerelTaken..full.))%>%
...
# subset to remove data from D from 2012-2017
...
pred.vars=c("Decimal.median.time","Hours","Block","Year",
            "sst","Fraction.moon.illuminated","Fishing.pressure.all.years")

# predictor variables Removed at first pass---
# Block and Zone, do we need both of them? Should block be a random effect to help with proxy
# Should Zone be included as a proxie for dories in the Pilbara vs no dories in the others
# or as a proxy for latitudinal change?
# Should block be included as continuous smoothed effect, showing the effect of latitude?
# in which case it would have to be changed to just a latitudinal variable
# Year, should that be categorical or continuous 
...
# Decided that hours, fishing.pressure.all.years needed a sqrt transformation

# # Re-set the predictors for modeling----
pred.vars=c("Decimal.median.time","sqrt.Hours","Block","Year",
            "sst","Fraction.moon.illuminated","sqrt.Fishing.pressure.all.years") 

# Check to make sure Response vector has not more than 80% zeros----
unique.vars=unique(as.character(dat$Zone))
unique.vars.use=character()
for(i in 1:length(unique.vars)){
  temp.dat=dat[which(dat$Zone==unique.vars[i]),]
...
setwd("G:/My Drive/Masters/Project/Spanish Mackerel Depredation/Results/R/Analysis_Carmody_depredation/Model outputs") #Set wd for example outputs - will differ on your computer
resp.vars=unique.vars.use
use.dat=dat
factor.vars=c("Month","Zone")# Month as a Factor with 12 levels, Zone as a factor with 3 levels
out.all=list()
var.imp=list()

# Loop through the FSS function for each Zone----
for(i in 1:length(resp.vars)){  
  png(file=paste(study,resp.vars[i],"mod_fits.png",sep="_")) # THis tells it to make
  # a png image file of the model fits output
  use.dat=dat[which(dat$Zone==resp.vars[i]),]
  
  Model1=gam(response~s(sst,k=3,bs='cr')+ s(Skipper,bs="re"),
